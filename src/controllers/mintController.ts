    ...
    if (req.audience) {
      ...
      if (paused) {
        res.status(503).json({
          ...
          message: 'New minting is temporarily paused (reserve ratio below 102%).',
        });
        return;
      }
      await checkDepositLimits(req.audience, usdcNum, userId, req.apiKey?.organizationId ?? null);
    }
...
export async function mintFromUsdcInternal(
  ...
  const tx = await prisma.transaction.create({
    data: {
      ...
      rateSnapshot: { source: 'xlm_on_ramp', timestamp: new Date().toISOString() },
    },
  });
...
const depositBodySchema = z.object({
  ...
  amount: z.string().min(1).refine((s) => !Number.isNaN(Number(s)) && Number(s) > 0, 'must be positive'),
  wallet_address: z.string().length(56).regex(/^G/),
});
...
export async function depositFromBasketCurrency(
  ...
  try {
    ...
    await logAudit({
      ...
      newValue: { type: 'mint', currency, amount: amountNum, wallet_address: wallet_address ? '***' : undefined },
    ...
    res.status(202).json({
      ...
      status: 'pending',
      message: 'Deposit received. Complete payment to the designated account for your currency; ACBU will be minted after confirmation.',
    });
  } catch (error) {
    next(error);
  }
